...
function howManyTimes(arr, toFind) {
	let total = 0
...
// 		return arr1
// 	}
// 	let final = arr1.filter((word) => {
// 		return arr2[0] !== word && arr2[1] !== word
// 	})
// 	return final
// }
...
function greatestProduct(arr) {
	...
	if (countTwo === matrixSize) {
		return 16
	}

	for (let i = 0; i < arr.length; i++) {
		for (let j = 0; j < arr[i].length - 1; j++) {
			let comp = arr[i][j] * arr[i][j + 1]
			if (comp > bigLine) {
				bigLine = comp
			}
		}
